  ...
  for (const file of fs.readdirSync('./test/sample_data/xccdf/input/STIG')) {
    for (const ruleId of ruleIdTypes) {
      it(`Successfully parses STIG ${file} using rule Id "${ruleId}"`, () => {
        const xccdfProfile = processXCCDF(fs.readFileSync(`./test/sample_data/xccdf/input/STIG/${file}`, 'utf-8'), false, ruleId as 'group' | 'rule' | 'version');
    ...
    for (const ruleId of ruleIdTypes) {
      if (!(file.includes('ssg-ubuntu') && ruleId.includes('cis'))
        ...
        it(`Successfully parses CIS Benchmark ${file} using rule Id "${ruleId}"`, () => {
          const xccdfProfile = processXCCDF(fs.readFileSync(`./test/sample_data/xccdf/input/CIS/${file}`, 'utf-8'), false, ruleId as 'group' | 'rule' | 'version' | 'cis');
...
describe('The XCCDF Parser', () => {
  it('Fails to parse CIS Benchmark ssg-ubuntu1604-xccdf using rule Id "cis" due to improper rule id format', () => {
    expect(() => {
      const xccdfProfile = processXCCDF(fs.readFileSync('./test/sample_data/xccdf/input/CIS/ssg-ubuntu1604-xccdf.xml', 'utf-8'), false, 'cis');
  ...
  it('Fails to parse CIS Benchmark ssg-ubuntu1804-xccdf using rule Id "cis" due to improper rule id format', () => {
    expect(() => {
      const xccdfProfile = processXCCDF(fs.readFileSync('./test/sample_data/xccdf/input/CIS/ssg-ubuntu1804-xccdf.xml', 'utf-8'), false, 'cis');
  ...
  it('Fails to parse CIS Benchmark ssg-ubuntu2004-xccdf using rule Id "cis" due to improper rule id format', () => {
    expect(() => {
      const xccdfProfile = processXCCDF(fs.readFileSync('./test/sample_data/xccdf/input/CIS/ssg-ubuntu2004-xccdf.xml', 'utf-8'), false, 'cis');
