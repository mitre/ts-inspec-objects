  ...
  for (const updatedValue in src) {
    ...
    if (existingValue !== undefined) {
      if (
        ...
        dst[updatedValue] = projectValuesOntoExistingObj(
          existingValue as Record<string, unknown>,
          src[updatedValue] as Record<string, unknown>,
          currentPath + updatedValue + '.'
        );
        ...
        const uniqueArrayValues = [
          ...new Set(
            (_.get(dst, updatedValue, []) as unknown[]).concat(
              src[updatedValue] as unknown[]
            )
          )
        ];
  ...
  if (control.code) {
    ...
    splitLines.forEach((line, lineIndex) => {
      ...
      if (typeof splitLines[lineIndex - 1] === 'string') {
        if (
          knownInSpecKeywords.indexOf(
            splitLines[lineIndex - 1].trim().split(' ')[0]
          ) !== -1 &&
...
export function findUpdatedControlByAllIdentifiers(
...
export function updateControl(
  ...
  logger.debug(
    `Existing describe block for control ${from.id}: ${JSON.stringify(
      existingDescribeBlock
    )}`
  );
  const projectedControl = projectValuesOntoExistingObj(
...
export function updateProfile(
  ...
  using: Profile,
  logger: winston.Logger
): Omit<UpdatedProfileReturn, 'markdown'> {
  ...
  diff.simplified.addedControlIDs.forEach((id) => {
    ...
    if (addedControl) {
      ...
      throw new Error(
        `New control ${id} added but don't have the control data`
      );
    }
  });
  ...
  for (const existingControl of from.controls) {
    const updatedControl = findUpdatedControlByAllIdentifiers(
      existingControl,
      using.controls
    );
    if (updatedControl) {
...
export function updateProfileUsingXCCDF(
  from: Profile,
  using: string,
  id: 'group' | 'rule' | 'version' | 'cis',
  logger: winston.Logger,
  ovalDefinitions?: Record<string, OvalDefinitionValue>
